...
public class SearchAdapter extends BaseQuickAdapter<PhotoData,BaseViewHolder>
{
    private Context mContext;

    public SearchAdapter(Context context,@Nullable List<PhotoData> datas)
    {
    ...
    protected void convert(BaseViewHolder helper,PhotoData photoData)
    {
        ...
        helper.addOnClickListener(R.id.item_search_favority);
        helper.addOnClickListener(R.id.item_search_favority_all);
        helper.getView(R.id.item_search_favority).setTag(photoData);
        helper.getView(R.id.item_search_favority_all).setTag(photoData);
        helper.setChecked(R.id.item_search_favority,photoData.getFavorite());
        /*******************************************************************/
        ...
        helper.getView(R.id.item_search_img).setLayoutParams(new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,photoData.getDownloadImgHeight()));/*************************/
        Glide.with(mContext).load(photoData.getDownloadThumbUrl()).apply(options).into((ImageView)helper.getView(R.id.item_search_img));/***************************************************/
        helper.setText(R.id.item_search_description,(null != photoData.getWidth() && !"".equals(photoData.getWidth().trim()) ?/*************************************************************/
