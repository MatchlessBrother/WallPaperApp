...
public class PhotoModel extends BaseMvp_PVModel
{
    public static final int GetPhotoDatas = 0x0001;
    ...
    public void executeOfNet(Context context, int netRequestCode, BaseMvp_LocalListCallBack localCallBack)
    {
        ...
        {
            case GetPhotoDatas:NetClient.getInstance(context).getNetUrl().getPhotoDatas(getForms()).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new BaseMvp_NetListCallBack(context,localCallBack));break;
        }
    }
