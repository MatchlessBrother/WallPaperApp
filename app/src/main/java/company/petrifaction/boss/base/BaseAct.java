package company.petrifaction.boss.base;
...
public abstract class BaseAct extends BaseActivity implements BaseMvp_View,View.OnClickListener
{
    ...
    protected void setTitleBackDrawable(int resource)
    {
        if(isUseDefaultTitleLine())
            mTitleBackBtn.setImageDrawable(getResources().getDrawable(resource));
        else
            Log.i(LOG_TAG,"因未使用自定义标题栏,所以设置返回按钮图标失败！");
    }
    ...
    protected void setTitleMoreDrawableIcon(int resource)
    {
        if(isUseDefaultTitleLine())
            mTitleMoreIcon.setImageDrawable(getResources().getDrawable(resource));
        else
            Log.i(LOG_TAG,"因未使用自定义标题栏,所以设置更多按钮图标失败！");
    }
    ...
    {
        return PromptBoxUtils.showLoadingDialog(this,"请稍等",2,15,false,null);
    }
    ...
    {
        return PromptBoxUtils.showLoadingDialog(this,"请稍等",2,15,isCanceledOnTouchOutside,onClickOutsideListener);
    }
    ...
    {
        return PromptBoxUtils.showPromptDialog(this,titleStr,Color.argb(255,51,51,51), 36,TypedValue.COMPLEX_UNIT_MM,new ColorDrawable(0xffffffff),
                                  View.VISIBLE,contentStr, Color.argb(255,102,102,102),30,TypedValue.COMPLEX_UNIT_MM,new ColorDrawable(0xffffffff),
